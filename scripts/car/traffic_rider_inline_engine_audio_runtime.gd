extends TrafficRiderInlineEngineAudioSynthesizer
class_name TrafficRiderInlineEngineAudioRuntime
...
func _ready() -> void:
	...
	if profile == null or not profile.validate().is_empty():
		push_error("TrafficRiderInlineEngineAudioRuntime requires a valid architecture profile.")
		set_process(false)
		return
