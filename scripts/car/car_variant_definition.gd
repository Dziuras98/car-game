...
func validate() -> PackedStringArray:
	...
	if variant_id == &"": errors.append("variant_id must not be empty")
	if get_menu_name().strip_edges().is_empty(): errors.append("variant menu name must not be empty")
	if sort_order < 0: errors.append("sort_order must be non-negative")
	if car_scene == null:
	...
	if specs == null:
		...
		for specs_error: String in specs.validate(): errors.append("specs: %s" % specs_error)
	if engine_label.strip_edges().is_empty(): errors.append("engine_label must not be empty")
	if drivetrain_label.strip_edges().is_empty(): errors.append("drivetrain_label must not be empty")
...
func get_menu_name() -> String:
	...
	if specs != null and specs.display_name != "": return specs.display_name
...
func is_ai_eligible_for_race() -> bool:
	return ai_eligible and car_scene != null and ai_car_scene != null and _scene_has_player_car_root(car_scene) and _scene_has_player_car_root(ai_car_scene) and specs != null and specs.is_valid() and specs.uses_geared_transmission()

func get_transmission_label() -> String:
	if specs == null: return ""
	var forward_gear_count: int = specs.gear_ratios.size()
	if specs.is_smg_transmission(): return "%d-speed SMG" % forward_gear_count
	if specs.is_manual_transmission(): return "%d-speed manual" % forward_gear_count
	if specs.is_automatic_transmission(): return "%d-speed automatic" % forward_gear_count
	if specs.is_cvt_transmission(): return "CVT"
	return "Direct drive"

func _scene_has_player_car_root(scene: PackedScene) -> bool:
	if scene == null or not scene.can_instantiate(): return false
...
func _get_scene_root_script(scene_state: SceneState, visited_states: Dictionary) -> Script:
	if scene_state == null: return null
	var state_id: int = scene_state.get_instance_id()
	if visited_states.has(state_id): return null
	...
	if scene_state.get_node_count() > 0:
		...
		if root_instance != null:
			...
			if instance_script != null: return instance_script
...
func _script_inherits(script: Script, expected_base: Script) -> bool:
	...
	while current_script != null:
		if current_script == expected_base: return true
