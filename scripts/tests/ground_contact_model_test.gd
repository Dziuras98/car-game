...
func _test_ground_contact_math() -> void:
	...
	var probes: Array[Vector3] = model.get_probe_local_positions(2.6, 1.5, 1.7, 0.4)
	_expect(probes.size() == 4, "ground-contact model produces four suspension probes")
	_expect(is_equal_approx(absf(probes[0].x - probes[1].x), 1.5), "front probes use the configured front track width")
	_expect(is_equal_approx(absf(probes[2].x - probes[3].x), 1.7), "rear probes use the configured rear track width")
...
func _test_partial_contact_support_scaling() -> void:
	...
	for expected_contact_count: int in range(1, 5):
		...
		chassis.update_tires(state, 0.0, false, car, null, 1.0 / 60.0)
		_expect(state.ground_contact_count == expected_contact_count, "runtime suspension reports exactly %d active probe contact(s)" % expected_contact_count)
		_expect(state.ground_normal.distance_to(Vector3.UP) < 0.001, "%d-contact support preserves the flat averaged normal" % expected_contact_count)
		if expected_contact_count == 1:
			...
			_expect(absf(state.suspension_acceleration - single_probe_support * expected_contact_count) < 0.02, "spring support is the explicit sum of %d active probe forces" % expected_contact_count)
...
func _create_surface(surface_name: String, position: Vector3, size: Vector3, grip_multiplier: float) -> TrackSurfaceBody:
