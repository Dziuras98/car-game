...
const LEGACY_TUNING_PROPERTIES: PackedStringArray = [
	"acceleration",
	"brake_deceleration",
...
func _run() -> void:
	...
	for violation: String in violations:
		push_error("[LEGACY_CONTROLLER_ACCESS_TEST] %s" % violation)
...
func _test_detector_fixtures() -> void:
	...
	_expect(violating_results.size() == 1, "detector catches a direct legacy-property access")
...
func _find_violations(source_text: String, source_path: String) -> Array[String]:
	...
	for variable_name_value: Variant in controller_variable_names.keys():
		...
		for property_name: String in LEGACY_TUNING_PROPERTIES:
...
func _expect(condition: bool, message: String) -> void:
	_checks += 1
	if condition:
		print("[LEGACY_CONTROLLER_ACCESS_TEST][PASS] %s" % message)
		return

	_failures.append(message)
	push_error("[LEGACY_CONTROLLER_ACCESS_TEST][FAIL] %s" % message)


func _finish() -> void:
	if _failures.is_empty():
		print("[LEGACY_CONTROLLER_ACCESS_TEST] Passed: %d checks" % _checks)
		quit(0)
		return

	push_error(
		"[LEGACY_CONTROLLER_ACCESS_TEST] Failed: %d failure(s), %d checks"
		% [_failures.size(), _checks]
	)
	for failure_message: String in _failures:
		push_error("[LEGACY_CONTROLLER_ACCESS_TEST] - %s" % failure_message)
	quit(1)
