...
const PROFILE_PATH: String = "res://resources/audio/370z_stock_audio_profile.tres"
...
func _initialize() -> void:
	var profile := load(PROFILE_PATH) as EngineAudioProfile
	_expect(profile != null, "the standard 370Z typed audio profile loads")
	if profile == null:
		_finish()
		return
	var engine_audio := EngineAudioSynthesizer.new()
	profile.apply_to(engine_audio)

	_expect(is_equal_approx(engine_audio.synthesis_gain_db, 1.0), "370Z profile limits internal saturation drive")
	_expect(is_equal_approx(engine_audio.output_volume_boost_db, 11.5), "370Z profile applies linear output compensation")
	...
	var idle_frames: PackedFloat32Array = engine_audio.generate_test_frames(FRAME_COUNT, 850.0, 0.0, 0.0)

	var no_load_output_rms: float = _rms(no_load_frames) * db_to_linear(engine_audio.idle_volume_db + engine_audio.output_volume_boost_db)
	var loaded_loudness: float = clampf(0.50 * 0.90 + 0.50 * 0.26, 0.0, 1.0)
	var loaded_volume_db: float = lerpf(engine_audio.idle_volume_db, engine_audio.load_volume_db, loaded_loudness) + engine_audio.output_volume_boost_db
	var loaded_output_rms: float = _rms(loaded_frames) * db_to_linear(loaded_volume_db)
	var idle_output_rms: float = _rms(idle_frames) * db_to_linear(engine_audio.idle_volume_db + engine_audio.output_volume_boost_db)

	print("[370Z_CLOSED_THROTTLE_AUDIO_TEST] idle_rms=%.7f no_load_3000_rms=%.7f loaded_3000_rms=%.7f ratio=%.3f" % [idle_output_rms, no_load_output_rms, loaded_output_rms, no_load_output_rms / maxf(loaded_output_rms, 0.000000001)])
