...
func _run() -> void:
	...
	_expect(not state.update_free_drive_car_variant(&"car"), "menu state rejects car switching")
	...
	_expect(state.update_free_drive_car_variant(&"manual"), "free-drive session accepts car switching")
	...
	_expect(not state.update_free_drive_car_variant(&"manual"), "race session rejects free-drive car switching")
