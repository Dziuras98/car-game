...
func _capture_spawn_signature(world: Node3D, spawn_marker: Node3D, track: GeneratedTrack, seed: int) -> Array[String]:
	var spawner: CarSpawner = CarSpawner.new()
	var configured: bool = spawner.configure(world, spawn_marker, track, CATALOG.get_all_variants(), 4.2, 7.0, seed)
