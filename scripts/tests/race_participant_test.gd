...
func _initialize() -> void:
	...
	_expect(player.is_valid(), "player participant requires a stable id and car reference")
	_expect(player.is_player(), "player participant exposes typed player kind")
	_expect(player.participant_id == &"player", "player participant uses a stable id")
	_expect(player.get_display_label() == "Ty", "player label is derived from participant kind")

	_expect(first_opponent.is_valid(), "opponent participant is valid independently of node naming")
	_expect(not first_opponent.is_player(), "opponent participant exposes typed opponent kind")
	_expect(first_opponent.participant_id == &"opponent_1", "first opponent uses a stable id")
	_expect(second_opponent.participant_id == &"opponent_2", "second opponent uses a distinct stable id")
	_expect(first_opponent.ordinal == 1 and second_opponent.ordinal == 2, "opponent order is explicit participant data")
	...
	second_opponent.display_name = "Gość specjalny"
	_expect(second_opponent.get_display_label() == "Gość specjalny", "explicit participant display name overrides the generated label")
	var clamped_opponent: RaceParticipant = RaceParticipant.create_opponent(opponent_car, 0)
	_expect(clamped_opponent.participant_id == &"opponent_1", "invalid opponent ordinal is clamped to the first stable slot")
