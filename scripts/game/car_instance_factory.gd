...
func _get_opponent_variant() -> CarVariantDefinition:
	...
	if _rng != null: selected_index = _rng.randi_range(0, _ai_eligible_variants.size() - 1)
	return _ai_eligible_variants[selected_index]

func _instantiate_variant_scene(variant: CarVariantDefinition, car_scene: PackedScene, scene_role: String) -> PlayerCarController:
	if variant == null: return null
	var specs: CarSpecs = variant.get_specs()
	if not _validate_specs(specs, "Car variant %s" % str(variant.variant_id)): return null
	if car_scene == null:
		push_error("Car variant %s must provide a %s car scene." % [str(variant.variant_id), scene_role])
		return null
	...
	if car_controller == null:
		push_error("Car variant %s %s scene must have PlayerCarController on its root node." % [str(variant.variant_id), scene_role])
...
func _validate_specs(specs: CarSpecs, context: String) -> bool:
	...
	if validation_errors.is_empty(): return true
