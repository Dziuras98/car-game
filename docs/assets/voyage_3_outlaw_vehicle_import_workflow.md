...
This document is the authoritative procedure for integrating every retained vehicle from the **Voyage 3: Outlaw Playable & NPC Vehicles** bundle into the Godot project.
...
This procedure copies every applicable research, approval, geometry, transmission, physics, audio and validation rule from PR #107. The source bundle is used only for the project's private, noncommercial scope under the accepted-risk decision in `docs/accepted_risks.md`. The asset notice in `THIRD_PARTY_NOTICES.md` must remain intact.
...
**No model may enter `integrating` until all included models have reached `approved`.** All 18 models currently remain `source_only`.

### Mandatory source-upload gate

The verified GLBs listed in `docs/assets/voyage_3_outlaw_source_upload.md` must be committed with exactly the recorded filenames and SHA-256 values before model research can leave `source_only`. This draft branch initially records the workflow and exact source contract; missing or mismatched source files are an explicit blocker, not permission to substitute another model.

### Mandatory physics dependency — PR #118

No Voyage 3: Outlaw vehicle implementation may begin until PR #118, **Rework per-wheel vehicle physics and recalibrate DPI v3**, is completed and its final physics work is integrated into `master`.
...
6. **Calibrate every model independently.** Scale must be based on verified dimensions for the represented body variant. No package-wide conversion is authoritative or may be reused as a default.
...
9. **Provide four independent wheel nodes.** Each paired axle mesh must be split into left and right wheel geometry with a hub-centred pivot. Multi-axle and dual-rear-wheel vehicles require every physical wheel position represented explicitly.
...
19. **Do not reintroduce excluded source representations.** The low-detail UAZ and Gazelle flatbed require a new explicit owner decision before they may be committed or implemented.

## Status gates

Use these statuses in the inventory and model-specific record:

1. `source_only` — source GLB is committed or its exact upload contract is recorded, but research has not started;
...
```text
assets/third_party/sketchfab/voyage_3_outlaw_vehicles/<vehicle_id>/processed/<vehicle_id>_godot.glb
```
...
Multi-axle, dual-rear-wheel or decorative-wheel source topology requires a model-specific wheel-node plan that names every physical wheel and separates non-physical decorative geometry.
...
After all 18 owner-scope decisions are complete, all verified source GLBs are committed, and the PR #118 dependency is satisfied, implementation proceeds by numeric source prefix, ascending. Geometry-class pilot work may validate shared tooling, but it does not override the owner-directed numeric order or permit implementation before any global gate is resolved.
