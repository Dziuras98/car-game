...
This document is the authoritative orchestration procedure for integrating every non-heavy vehicle from the **Traffic Rider NPC Vehicles** bundle into the Godot project.
...
The detailed Stage 6 and Stage 8 methods are mandatory parts of this workflow:
...
The repository contains 20 included source GLBs. The following large-truck models remain outside the approved scope:
...
All 20 owner-scope decisions are complete. The approved combined scope is recorded in `docs/assets/traffic_rider_npc_vehicle_inventory.md` and totals 285 mechanically consolidated configurations.

## Satisfied global gates and authoritative baseline

### Research-before-implementation gate

The historical rule was: complete research and obtain owner-scope approval for **every included model before any implementation begins**. An individual model reaching `approved` fixed its scope but did not independently authorize geometry, catalog, physics, transmission or audio work.

This condition is now satisfied for all 20 included models.

### Physics dependency — PR #118

The historical implementation dependency required PR #118, **Rework per-wheel vehicle physics and recalibrate DPI v3**, or an explicitly identified merged successor, to be integrated before any Traffic Rider implementation.

This dependency is now satisfied. PR #118 was merged into `master`, and the authoritative implementation baseline is:

`3743f5e95391b63a97e81b95050984b8240b7f30`

The branch was synchronized to that baseline and the reviewed interfaces are recorded in `docs/assets/traffic_rider_npc_vehicle_physics_v3_baseline.md`.

The original gate remains part of the audit trail. It required:

1. merged authoritative physics work;
2. branch synchronization;
3. recorded physics commit;
4. review of per-wheel contact, slip, load transfer, inertia, differentials, AWD, braking, steering/yaw, drag, transmissions and DPI;
5. updated shared/model-specific assumptions;
6. a green current test suite before model 01 entered `integrating`.

A future shared physics change does not reopen owner scope, but it does require the Stage 7 synchronization and recalibration procedure.
...
3. **Retain the research as structured input.** Previously established engines, curves, masses, ratios, transmission identities, tyres, aerodynamics and targets must be migrated, not researched again or replaced by guesses.
4. **Keep the committed source GLB unchanged.** Technical adaptations belong in a deterministic derived GLB, Godot wrapper, material override or project-authored resource.
5. **Calibrate every model independently.** A package-wide conversion factor is not authoritative.
6. **Use wheelbase as the primary scale reference.** Cross-check overall dimensions, track, height and tyre diameter.
7. **Use the project coordinate convention.** `+Y` is up, local `-Z` is front, local `-X` is left and local `+X` is right.
8. **Provide four independent wheel nodes.** Split paired axle meshes into left/right hub-centred geometry.
9. **Use explicit wheel bindings.** Do not add generic name scanning or heuristic wheel discovery to `CarVisualController`.
10. **Match the real transmission architecture exactly.** A torque-converter planetary automatic, automated manual, DCT, CVT, conventional manual and electric reduction each require correct mechanics and semantics.
11. **Implement missing transmission types faithfully.** A missing architecture requires a dedicated model or clean shared extension, not a fallback.
12. **Treat the transmission as a complete torque path.** Ratios and shift delay alone do not implement launch devices, shift phases, lock-up, preselection, transfer cases or AWD couplings.
13. **Reproduce performance from evidence, not labels.** Torque curves, gearing, mass, drag, tyres, losses, shift behaviour and physical limits must produce the result.
14. **Build new engine-sound architectures from first principles.** Cylinder layout, crank/firing sequence, banks/collectors, combustion, aspiration and transients define the backend. A profile, pitch shift or EQ does not change architecture.
15. **Keep every model compatible with current `master` physics.** Recalibrate affected models rather than adding compatibility hacks.
16. **Separate visual and physical data.** Collision, contact, mass, powertrain and traffic behaviour are authored data, not runtime render-mesh inference.
17. **Document uncertainty.** Evidence-blocked rows remain approved in scope but unavailable until complete; guessed parameters are prohibited.
18. **One model is complete only after full validation.** An imported GLB alone is not an integrated vehicle.

## Status gates

Use these statuses in the inventory and model record:

1. `source_only` — source committed; research not started;
2. `researching` — identity, dimensions and full powertrain matrix being established;
...
A model must not skip `awaiting_owner_scope`. No row may be exposed merely because the parent model is `integrating`.

## Repository layout

Move the source only when its numbered implementation begins, preserving bytes and updating references in the same commit:
...
```text
assets/third_party/sketchfab/traffic_rider_npc_vehicles/<vehicle_id>/processed/<vehicle_id>_processed.glb
```
...
- body/chassis/trim restriction when material;
- engine family/code, fuel, aspiration, displacement, cylinder layout;
...
- drivetrain, transfer case, coupling and differential state;
...
1. manufacturer technical/homologation data;
...
Classify every combination/value as `verified_factory`, `strongly_supported`, `provisional`, evidence-blocked or `rejected/not_factory`. Preserve conflicts and resolve them explicitly.
...
- total combinations;
- engine families/calibrations;
- exact transmission architectures/codes;
- drivetrain layouts;
- regional/year-only variants;
- unresolved rows;
...
2. retain its engine/calibration record;
3. retain curves, mass, gearing, tyre, aero and validation data;
4. retain explicit evidence states for missing exact values;
5. prove the transmission and audio architecture decisions;
6. keep incomplete rows unavailable.

Follow `traffic_rider_npc_vehicle_research_data_contract.md`.
...
Record path/SHA-256, root/mesh names, triangles, surfaces, materials/textures, AABB, axes, ground offset, axle/wheel centres, pivots, transparency, transforms and detached geometry. Document any topology exception.
...
Use a shared data-driven controller where appropriate. Provide explicit body/four-wheel paths, hub-centred pivots, visibility AABB, wheel radius and LOD decision. Test independent steering/spin state for all physical wheels. Do not add runtime name heuristics.

## Stage 5 — build approved model and variant data

Create exactly owner-approved rows. Migrate rather than reconstruct full curves, idle/redline/limiter, ratios, mass/load distribution, drag, tyres, brakes, drivetrain and targets. Do not merge mechanically distinct calibrations or create marketing-alias duplicates.
...
The minimum rule remains:

- classic automatic is not automated manual;
- automated manual retains the manual gearset/clutch interruption;
...
- portal/e-axle reductions affect torque, speed and inertia.
...
4. identify affected models/rows;
5. rerun full tests;
...
Research and implement cylinder layout, crank/firing order/intervals, banks/collectors, intake/exhaust, combustion/injection, aspiration, mechanical sources and transients. A new physical architecture requires an appropriate procedural source model; profiles calibrate a correct architecture but do not transform an unrelated waveform.
...
Limiter torque cut is not pedal lift: it must not erase turbo state or trigger false release audio. Added turbo layers must not make the complete engine exceed the loudness reference merely through gain stacking.
...
- dimensions, orientation, collision and four-wheel animation;
...
- LOD/runtime/fleet budgets;
- shared/catalog-wide regressions;
...
- structured data/evidence state;
...
Implementation proceeds by numeric source prefix. Model 02 remains queued until model 01 reaches `integrated`. Shared architecture work created for an earlier model may be reused later only when the detailed transmission/audio contracts prove semantic compatibility.
